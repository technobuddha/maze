    ...
    if (this.drawing) {
      ...
      switch (direction) {
        ...
        case 'b': {
          ...
          const { x: x0, y: y0 } = toCartesian({ r: r2, φ: toRadians(ab) });
          const { x: x1, y: y1 } = toCartesian({ r: r2, φ: toRadians(ae) });
          const { x: x2, y: y2 } = toCartesian({ r: r3, φ: toRadians(af) });
          const { x: x3, y: y3 } = toCartesian({ r: r3, φ: toRadians(ac) });
        ...
        case 'd': {
          ...
          const { x: x0, y: y0 } = toCartesian({ r: r2, φ: toRadians(a5) });
          const { x: x1, y: y1 } = toCartesian({ r: r2, φ: toRadians(a2) });
          const { x: x2, y: y2 } = toCartesian({ r: r3, φ: toRadians(a1) });
          const { x: x3, y: y3 } = toCartesian({ r: r3, φ: toRadians(a4) });
  ...
  public drawX(cell: Cell, color = this.color.blocked): void {
    if (this.drawing) {
      if (this.zones[cell.y] === 1) {
        ...
        const { x: x0, y: y0 } = toCartesian({ r: r2, φ: toRadians(a5) });
        const { x: x1, y: y1 } = toCartesian({ r: r2, φ: toRadians(ab) });
        const { x: x2, y: y2 } = toCartesian({ r: r3, φ: toRadians(ac) });
        const { x: x3, y: y3 } = toCartesian({ r: r3, φ: toRadians(a4) });
